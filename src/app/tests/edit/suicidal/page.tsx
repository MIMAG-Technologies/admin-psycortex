...
const SUICIDAL_OPTIONS = ["Strongly Disagree", "Disagree", "Uncertain", "Agree", "Strongly Agree"];
const POSITIVE_SCORES = [1, 2, 3, 4, 5];
const NEGATIVE_SCORES = [5, 4, 3, 2, 1];

const getQuestions = async () => {
    try {
        const response = await axios.get(`${base_url}/edits/suicidal/get_questions.php`);
...
const getInterpretations = async () => {
    try {
        const response = await axios.get(`${base_url}/edits/suicidal/get_interpretations.php`);
...
const updateQuestions = async (questions: Question[]) => {
    try {
        const response = await axios.post(`${base_url}/edits/suicidal/set_questions.php`, questions);
...
const updateInterpretations = async (interpretations: Interpretation[]) => {
    try {
        const response = await axios.post(`${base_url}/edits/suicidal/set_interpretations.php`, interpretations);
        return response.data;
    } catch (error) {
...
export default function SuicidalEditPage() {
    ...
    return (
        <div className="container mx-auto p-6 max-w-7xl">
            <h1 className="text-3xl font-bold mb-8 text-primary">
                Suicidal Tendencies Test Management
            </h1>
            ...
            <ConfirmationModal
                ...
                description=""
                testName="Suicidal Tendencies Test"
                                            ...
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="text-xs text-muted-foreground">
                                                    {question.type === "positive"
                                                        ? "Positive Question: Higher score = higher suicidal tendencies"
                                                        : "Negative Question: Lower score = higher suicidal tendencies"}
                                                </div>
                                            </div>
                                            {SUICIDAL_OPTIONS.map((option, optionIndex) => (
