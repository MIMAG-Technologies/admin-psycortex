...
export default function MaritalEditingPage() {
    ...
    const MARITAL_OPTIONS = ["Always", "Sometimes", "Never"];
    ...
    const getQuestions = async () => {
        try {
            const response = await axios.get(`${base_url}/edits/marital/get_questions.php`);
    ...
    const getInterpretations = async () => {
        try {
            const response = await axios.get(`${base_url}/edits/marital/get_interpretations.php`);
    ...
    const updateQuestions = async (questions: Question[]) => {
        try {
            const response = await axios.post(`${base_url}/edits/marital/set_questions.php`, questions);
    ...
    const updateInterpretations = async (interpretations: Interpretation[]) => {
        try {
            const response = await axios.post(`${base_url}/edits/marital/set_interpretations.php`, interpretations);
            return response.data;
        } catch (error) {
    ...
    const getOptionScore = (optionIndex: number, isPositive: boolean): number => {
        // For positive questions: Always=2, Sometimes=1, Never=0
        // For negative questions: Always=0, Sometimes=1, Never=2
        if (isPositive) {
            return 2 - optionIndex;
        } else {
            return optionIndex;
        }
    };
                                ...
                                <div className="flex items-start gap-6">
                                    ...
                                    <div className="w-24 text-sm text-muted-foreground">
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <div className="text-sm">3-point Scale</div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-2 mt-4">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="text-xs text-muted-foreground">
                                                    {question.is_positive ? "Positive Scoring (2→0)" : "Negative Scoring (0→2)"}
                                                </div>
                                            </div>
                                            {MARITAL_OPTIONS.map((option, optionIndex) => (
                            ...
                            >
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <Textarea
                                            value={interpretation.recommendation_points}
                                            onChange={(e) =>
                                                handleInterpretationUpdate(
                                                    index,
