...
export default function BDIEditingPage() {
    ...
    const getQuestions = async () => {
        try {
            const response = await axios.get(`${base_url}/edits/bdi/get_questions.php`);
    ...
    const getInterpretations = async () => {
        try {
            const response = await axios.get(`${base_url}/edits/bdi/get_interpretations.php`);
    ...
    const updateQuestions = async (questions: Question[]) => {
        try {
            const response = await axios.post(`${base_url}/edits/bdi/set_questions.php`, questions);
    ...
    const updateInterpretations = async (interpretations: Interpretation[]) => {
        try {
            const response = await axios.post(`${base_url}/edits/bdi/set_interpretations.php`, interpretations);
            return response.data;
        } catch (error) {
    ...
    const parseOptions = (optionsString: string): QuestionOptions => {
        try {
    ...
    const handleOptionUpdate = (
        ...
        // Stringify the updated options
        const updatedOptionsString = JSON.stringify(options);

        // Update the question with new options string
        handleQuestionUpdate(questionIndex, "options", updatedOptionsString);
    };
    ...
    return (
        <div className="container mx-auto p-6 max-w-7xl">
            <h1 className="text-3xl font-bold mb-8 text-primary">
                Beck Depression Inventory (BDI) Management
            </h1>
            ...
            <ConfirmationModal
                ...
                description=""
                testName="Beck Depression Inventory (BDI)"
                ...
                <TabsContent value="questions">
                    <div className="grid gap-6">
                        {questions.map((question, index) => {
                            const options = parseOptions(question.options);
                            const optionKeys = Object.keys(options).sort((a, b) => Number(a) - Number(b));
                                                ...
                                                {optionKeys.map((key) => (
                                                    <div key={key} className="flex gap-2 items-start">
                                                        ...
                                                        <Textarea
                                                            ...
                                                            placeholder={`Response for score ${key}...`}
                                                        />
                                                    </div>
                                                ))}
                                            </div>
