...
export default function HappinessEditingPage() {
    ...
    const getQuestions = async () => {
        try {
            const response = await axios.get(`${base_url}/edits/happiness/get_questions.php`);
    ...
    const getInterpretations = async () => {
        try {
            const response = await axios.get(`${base_url}/edits/happiness/get_interpretations.php`);
    ...
    const updateQuestions = async (questions: Question[]) => {
        try {
            const response = await axios.post(`${base_url}/edits/happiness/set_questions.php`, questions);
    ...
    const updateInterpretations = async (interpretations: Interpretation[]) => {
        try {
            const response = await axios.post(`${base_url}/edits/happiness/set_interpretations.php`, interpretations);
            return response.data;
        } catch (error) {
    ...
    const getOptionScore = (optionIndex: number, isPositive: boolean): number => {
        // For positive questions: first option (index 0) = 5, last option (index 4) = 1
        // For negative questions: first option (index 0) = 1, last option (index 4) = 5
        return isPositive ? 5 - optionIndex : optionIndex + 1;
    };

    const getOptionFieldName = (optionIndex: number): keyof Question => {
        const fieldNames: Array<keyof Question> = [
            "option1_text",
            "option2_text",
            "option3_text",
            "option4_text",
                                            ...
                                            <div className="space-y-2 mt-4">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className="text-xs text-muted-foreground">
                                                        {question.is_positive ? "Positive Scoring (5→1)" : "Negative Scoring (1→5)"}
                                                    </div>
                                                </div>
                                                    ...
                                                    <div key={optionIndex} className="flex gap-2 items-start">
                                                        ...
                                                        <Textarea
                                                            ...
                                                            onChange={(e) =>
                                                                handleQuestionUpdate(
                                                                    index,
                                                                    getOptionFieldName(optionIndex),
                                                                    e.target.value
                                                                )
                                                            }
